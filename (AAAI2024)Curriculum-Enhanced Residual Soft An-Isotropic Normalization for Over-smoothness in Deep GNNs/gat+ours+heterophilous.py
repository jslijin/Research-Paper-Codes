...
def f(hidden_layer,hidden_channel,param_prop,param_before,param_first,param_ani,lr,weight_decay,lr_decay,epoch,dropout,PPR_num,lamda1,lamda2,gamma,knn_num,label_beta,d_0_rate,an_beta,head_num):
    ...
    class Pre_Train(torch.nn.Module):
        ...
        def forward(self,data,emb,dataset,knn_num):
            N,d=data.x.size()
            edge_index,edge_weight=torch.load("./Knngraph_feature/"+"knngraph_"+dataset+"_"+str(knn_num)+"neighbors")
    ...
    class Net(torch.nn.Module):
        def __init__(self):
            ...
            self.gcn_layers.append(dglnn.GATConv(data.num_node_features,hidden_channel//head_num,num_heads=head_num))         
            for i in range(hidden_layer-2):
                self.gcn_layers.append(dglnn.GATConv(hidden_channel,hidden_channel//head_num,num_heads=head_num))
            self.conv2 = dglnn.GATConv(hidden_channel, int(max(data.y) + 1),num_heads=1)          
        ...
        def forward(self,data):
            ...
            for i,layer in enumerate(self.gcn_layers) :       
                if i>0:  
                    ...
                    x=(param_prop/s)*x+(param_before/s)*x_origin+(param_first/s)*x_first
                else:
                    x=layer(edge_index,x)    
                    x=x.reshape(x.size(0),x.size(1)*x.size(2))
...
d={'hidden_layer': 32, 'hidden_channel': 64, 'param_prop': 0.1, 'param_before': 0.01, 'param_first': 0.7, 'param_ani': 0.01, 'lamda1': 0.1, 'lamda2': 0.4, 'gamma': 2, 'epoch': 300, 'PPR_num': 100, 'knn_num': 2, 'label_beta': 0.4, 'weight_decay': 5e-06, 'lr_decay': 150, 'lr': 0.01, 'dropout': 0.3, 'an_beta': 0.8, 'd_0_rate': 0.99,'head_num':4}
