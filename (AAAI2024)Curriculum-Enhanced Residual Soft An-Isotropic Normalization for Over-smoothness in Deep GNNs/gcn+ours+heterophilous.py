...
def f(hidden_layer,hidden_channel,param_prop,param_before,param_first,param_ani,lr,weight_decay,lr_decay,epoch,dropout,PPR_num,lamda1,lamda2,gamma,knn_num,label_beta,d_0_rate,an_beta):
            ...
            with graph.local_scope():
                ...
                if self._cached_h is not None:
                    ...
                    for _ in range(self._k):
                        ...
                        # feat_tmp=(1-label_beta)*feat+label_beta*feat_first
                        # feat_total=torch.cat((feat_total,feat_tmp.unsqueeze(0)),dim=0)
    ...
    class Net(torch.nn.Module):
        def __init__(self):
            ...
            self.gcn_layers.append(dglnn.GraphConv(data.num_node_features,hidden_channel))         
            for i in range(hidden_layer-2):
                self.gcn_layers.append(dglnn.GraphConv(hidden_channel,hidden_channel))
            self.conv2 = dglnn.GraphConv(hidden_channel, int(max(data.y) + 1))      
...
d={'hidden_layer': 64, 'hidden_channel': 64, 'param_prop': 0.1, 'param_before': 0.01, 'param_first': 0.7, 'param_ani': 0.01, 'lamda1': 0.1, 'lamda2': 0.4, 'gamma': 2, 'epoch': 300, 'PPR_num': 100, 'knn_num': 2, 'label_beta': 0.4, 'weight_decay': 5e-06, 'lr_decay': 150, 'lr': 0.01, 'dropout': 0.3, 'an_beta': 0.8, 'd_0_rate': 0.99}
